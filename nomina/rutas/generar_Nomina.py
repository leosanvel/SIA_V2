...
from rh.gestion_empleados.modelos.empleado import rEmpleado, tPuesto, rEmpleadoPuesto, rBancoPersona
...
def crear_Nomina():
    ...
    Cuenta_verificada = db.session.query(rEmpleado).join(rBancoPersona).filter(rEmpleado.idTipoEmpleado=="2",rEmpleado.Activo==1,rBancoPersona.Verificado==0).all()    
    ...
    #-----------Deducciones                
                ...
                for EConcepto in EmpleadoConcepto:
                    ...
                    existeregistro = db.session.query(rNominaPersonas).filter_by(idNomina = NumNomina,idPersona=EConcepto.idPersona,idTipoConcepto=EConcepto.idTipoConcepto,idConcepto=EConcepto.idConcepto).first()
                    if existeregistro:
                        existeregistro.Importe = existeregistro.Importe + Importe
                    else:
                        nuevoregistro = rNominaPersonas(NumNomina,EConcepto.idPersona,centro_costo,nivel_salarial,EConcepto.idTipoConcepto,EConcepto.idConcepto,Importe)
