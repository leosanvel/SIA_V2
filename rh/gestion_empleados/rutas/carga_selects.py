...
def select_TipAlt():
    ...
    TipAlts = db.session.query(TipoAlta).filter_by(idTipoEmpleado=TipEmpl, Activo = 1).all()
...
def select_Grupo():
    ...
    ret = '<option value="0">-- Seleccione --</option>'
    Grupos = db.session.query(Grupo).filter_by(idTipoAlta=TipAlt, Activo = 1).all()
    for entry in Grupos:
...
def select_municipio():
    ...
    ret = '<option value="0">-- Seleccione --</option>'
    municipios = db.session.query(Municipio).filter_by(idEntidad = Entidad, Activo = 1).all()
...
def select_localidad():
    ...
    ret = '<option value="0">-- Seleccione --</option>'
    localidades = db.session.query(Localidad).filter_by(idEntidad = Entidad, idMunicipio = Municipio, Activo = 1).all()
...
def select_asentamiento():
    ...
    ret = '<option value="0">-- Seleccione --</option>'
    Asentamientos = db.session.query(CodigoPostal).filter_by(idEntidad = Entidad, idMunicipio = Municipio, idTipoAsentamiento = TipoAsentamiento, Activo = 1).all()
...
def cargar_Plaza():
    ...
    ret = '<option value="0">-- Seleccione --</option>'
    Plazas = db.session.query(Puesto).filter_by(idCentroCosto = idCentroCosto).all()
    for entry in Plazas:
...
def cargar_EstNivEsc():
    ...
    ret = '<option value="0">-- Seleccione --</option>'
    NivEscs = db.session.query(NivelEscolaridad).outerjoin(EscolaridadNivel, NivelEscolaridad.idNivelEscolaridad == EscolaridadNivel.idNivelEscolaridad).filter(EscolaridadNivel.idEscolaridad == NivEsc, NivelEscolaridad.Activo == 1).order_by(NivelEscolaridad.NivelEscolaridad).all()
...
def cargar_Escuela():
    ...
    ret = '<option value="0">-- Seleccione --</option>'
    Escuelas = db.session.query(InstitucionEscolar).outerjoin(EscolaridadInstitucion, InstitucionEscolar.idInstitucionEscolar == EscolaridadInstitucion.idInstitucionEscolar).filter(EscolaridadInstitucion.idEscolaridad == NivEscolar, InstitucionEscolar.Activo == 1).order_by(InstitucionEscolar.InstitucionEscolar).all()
...
def cargar_FormacionEducativa():
    ...
    ret = '<option value="0">-- Seleccione --</option>'
    formaciones = db.session.query(FormacionEducativa).outerjoin(InstitucionFormacion, FormacionEducativa.idFormacionEducativa == InstitucionFormacion.idFormacionEducativa).filter(InstitucionFormacion.idInstitucionEscolar == institucionEscolar, FormacionEducativa.Activo == 1).order_by(FormacionEducativa.FormacionEducativa).all()
